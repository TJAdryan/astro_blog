...
tags: ["Data Science", "Medallion Architecture", "GWAS Catalog", "Data Engineering"]
...
When you look at the way raw scientific data is collected, it's amazing that any insights are ever drawn from it. It's often messy, inconsistent, and full of gaps. This is why much of the rigor in data science comes from taking the time to draw out insights over several repeatable, tested, and reliable processes. This is the essence of the **Medallion Architecture**, a design pattern that can be thought of as a “data polishing” pipeline that refines data at every step.

The Medallion Architecture organizes a data lake into three distinct layers. Each layer adds more structure and value to the data, transforming it from a raw, chaotic state into a clean, trustworthy resource.

***

### The Bronze Layer: The Raw Collection

The Bronze layer is where all the raw materials arrive. For a project like ours using the **GWAS Catalog**, its initial value is in its sheer scale. This catalog is a massive, centralized repository of thousands of genetic studies, saving scientists from having to manually sift through countless scientific papers.

However, the raw data still needs to be polished. The GWAS Catalog pulls findings from many different sources, which can lead to inconsistencies. Some records might be missing data, others might have formatting errors, and since genotyping using new array technologies is often associated with spurious results, the data needs a rigorous quality control check. The Bronze layer ensures we have a complete, untouched copy of the raw data before any cleaning begins, which is the first step toward reproducibility.
...
In our project, we took the clean data from the Silver layer and aggregated it to find the number of published studies for each genetic trait. This transformation gave us our key insight: a list of the most-studied traits in the entire GWAS Catalog. The final output is a small, easy-to-use file that is perfect for generating a chart or for a final report.
...
Bronze vs. Gold: A Visual Comparison
This is a great way to highlight the value added by each layer of the Medallion Architecture.

The raw data from the Bronze layer is a single, unstructured string of text. It's difficult to read and nearly impossible to analyze without extensive cleaning. It contains a mix of dates, journal names, numbers, and long descriptions. The value is there, but it's buried in chaos.
...
I added the full readme here for people who like things the pregit way:
